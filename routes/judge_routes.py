...
def api_participant_detail(gala_id: int, gala_categorie_id: int, participant_id: int):
    ...
    participant_row = conn.execute(
        """
        SELECT p.id, comp.nom AS compagnie_nom, comp.ville, comp.secteur,
               comp.responsable_nom, comp.responsable_titre
        FROM participant AS p
        JOIN compagnie AS comp ON comp.id = p.compagnie_id
    ...
    percent = round((completed / total_questions) * 100, 1) if total_questions else 0.0
...
def api_update_note(gala_id: int, gala_categorie_id: int, participant_id: int, question_id: int):
    ...
    question_exists = conn.execute(
        "SELECT 1 FROM question WHERE id = ? AND gala_categorie_id = ?",
    ...
    existing = conn.execute(
        ...
        (juge_id, participant_id, question_id),
    ).fetchone()
    ...
    conn.execute(
        ...
        """,
        (juge_id, participant_id, question_id, valeur_to_save, commentaire_to_save),
    )
    conn.commit()
